...
resource "aws_lambda_permission" "analysePermission" {
  ...
  source_arn = "arn:aws:execute-api:${var.region}:${var.acc_id}:${aws_api_gateway_rest_api.anl.id}/*/${aws_api_gateway_method.anlGetMethod.http_method}${aws_api_gateway_resource.anlGet.path}"
}
